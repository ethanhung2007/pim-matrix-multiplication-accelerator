import util_pkg::*;

module top_controller #(
    parameter int M = 8,
    parameter int N = 8,
    parameter int TILE_K = 64,
    parameter int DATA_W = 16,
    parameter int NUM_TILES = 8,
    parameter int ACC_W = 2 * DATA_W + $clog2(TILE_K)
) (
    input logic clk,
    input logic go,
    input logic rst,
    input logic valid_out,
    input logic [ACC_W + $clog2(NUM_TILES)-1:0] fsum,
    input logic [DATA_W-1:0] a_mem_data[NUM_TILES-1:0],
    input logic [DATA_W-1:0] b_mem_data[NUM_TILES-1:0],
    output logic a_we,
    output logic b_we,
    output logic start,
    output logic [$clog2(M * TILE_K * NUM_TILES)-1:0] a_mem_addr[NUM_TILES-1:0],
    output logic [$clog2(TILE_K * NUM_TILES * N)-1:0] b_mem_addr[NUM_TILES-1:0],
    output logic [$clog2(TILE_K)-1:0] wr_addr,
    output logic [DATA_W-1:0] a_wdata[NUM_TILES-1:0],
    output logic [DATA_W-1:0] b_wdata[NUM_TILES-1:0],
    output logic done,
    output logic [$clog2(M * N)-1:0] c_mem_addr,
    output logic [ACC_W + $clog2(NUM_TILES)-1:0] c_mem_data,
    output logic bank_sel,
    output logic c_wr_en
);

  localparam int K = TILE_K * NUM_TILES;
  
  

  typedef enum logic [2:0] {
    IDLE,
    PRIME,
    RUN,
    OUTPUT,
    DONE
  } state_t;

  state_t state, next_state;

  logic [$clog2(M)-1:0] i;
  logic [$clog2(N)-1:0] j;
  logic [$clog2(TILE_K):0] load_counter;
  logic [$clog2(TILE_K)-1:0] wr_addr_r;
  logic a_we_r, b_we_r;
  logic load_done;

  logic start_r;
  logic final_tile_write;
  logic final_prime_write;
  logic b_run_we_r;
  logic final_run_write;

  assign final_prime_write = a_we_r && b_we_r && (wr_addr_r == TILE_K-1);
  assign final_tile_write = b_we_r && (wr_addr_r == TILE_K-1);
  assign final_run_write = b_run_we_r && (wr_addr_r == TILE_K-1);

  always_comb begin
    next_state = state;
    case (state)
      IDLE: begin
        if (go) next_state = PRIME;
      end
      PRIME: begin
        next_state = (load_counter == TILE_K - 1) ? RUN : PRIME;
      end
      RUN: begin
        if (valid_seen && (load_done || j == N-1)) next_state = OUTPUT;
      end
      OUTPUT: begin
        if (i == M-1 && j == N-1)  next_state = DONE;
        else if (j == N - 1)       next_state = PRIME;  
        else                       next_state = RUN;
      end
      DONE: begin
        next_state = IDLE;
      end
      default: begin
        next_state = IDLE;
      end
    endcase
  end

  always_ff @(posedge clk) begin
    if (rst) begin
      state <= IDLE;
      load_counter <= 0;
      i <= '0;
      j <= '0;
      a_we_r <= '0;
      b_we_r <= '0;
      wr_addr_r <= '0;
      bank_sel <= 0;
      start_r <= 0;
      b_run_we_r <= 0;
    end else begin
      start_r <= final_tile_write;
    
      wr_addr_r <= load_counter;
      a_we_r <= (state == PRIME) && (j == 0) && (load_counter < TILE_K);
      b_we_r <= ((state == PRIME) || (state == RUN && j != N-1)) && (load_counter < TILE_K);
      b_run_we_r <= (state == RUN && j != N-1) && (load_counter < TILE_K);
      state <= next_state;
      if ((state == RUN || state == PRIME) && load_counter < TILE_K) load_counter <= load_counter + 1;
      else load_counter <= '0;
      if (state == OUTPUT) begin
        if (j == N - 1) begin
          j <= 0;
          i <= i + 1;
        end else begin
          j <= j + 1;
        end
      end
      if ((state == OUTPUT && j != N-1) || final_prime_write)
        bank_sel <= ~bank_sel;
    end
  end

  always_ff @(posedge clk) begin
    if (rst) begin
      load_done <= 0;
    end else if (state == IDLE || state == PRIME || state == OUTPUT) begin
      load_done <= 0;
    end else if (final_run_write) begin
      load_done <= 1;
    end
  end
  

  logic valid_seen;
  always_ff @(posedge clk) begin
    if (rst) begin
      valid_seen <= 0;
    end else if (state == OUTPUT || state == PRIME || state == IDLE) begin
      valid_seen <= 0;
    end else if (state == RUN && valid_out) begin
      valid_seen <= 1;
    end
  end

  logic [ACC_W + $clog2(NUM_TILES)-1:0] fsum_latched;
  always_ff @(posedge clk) begin
    if (rst) begin
      fsum_latched <= '0;
    end else if (state == RUN && valid_out) begin
      fsum_latched <= fsum;
    end
  end

  always_comb begin
    start = start_r;
    done = 0;
    c_wr_en = 0;
    a_mem_addr = '{default: '0};
    b_mem_addr = '{default: '0};
    c_mem_addr = '0;
    c_mem_data = '0;
    a_wdata = '{default: '0};
    b_wdata = '{default: '0};
    a_we = a_we_r;
    b_we = b_we_r;
    wr_addr = wr_addr_r;
    for (int t = 0; t < NUM_TILES; t++) begin
      if (a_we_r) a_wdata[t] = a_mem_data[t];
      if (b_we_r) b_wdata[t] = b_mem_data[t];
    end

    case (state)
      IDLE: begin
      end
      PRIME: begin
        for (int t = 0; t < NUM_TILES; t++) begin
          if (j == 0) a_mem_addr[t] = i * K + t * TILE_K + load_counter;
          b_mem_addr[t] = (t * TILE_K + load_counter) * N + j;
        end
      end
      RUN: begin
        if (load_counter < TILE_K && j != N - 1) begin
          for (int t = 0; t < NUM_TILES; t++)
            b_mem_addr[t] = (t * TILE_K + load_counter) * N + (j + 1);
        end
      end
      OUTPUT: begin
        c_wr_en = 1;
        c_mem_data = fsum_latched;
        c_mem_addr = i * N + j;
      end
      DONE: begin
        done = 1;
      end
    endcase
  end


endmodule
